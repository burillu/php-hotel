...
if (isset($_GET["parking"]) && isset($_GET["vote"])) {
  ...
  if ($_GET["parking"] === 'true') {
    ...
    //var_dump($parking);
    if (is_int($vote) && !is_null($parking)) {
...
<main>
  <div class="container">
    ...
    <?php if(isset($vote) && is_int($vote)){
     echo 'Hai selezionato Hotel con valutazione superiore a ' .($vote + 1) ;
    } ?>
    <?php
    if (count($hotels) > 0) { ?>
      <table class="table">
    ...
    <?php } else {?>
      <div class="alert alert-danger">
        Ci dispiace, la ricerca non ha prodotto alcun risultato.
      </div>
